...
function renderTable() {
  ...
  if (!filteredRows.length) {
    tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:60px;color:#8a8278">לא נמצאו בקשות</td></tr>';
    return;
  }
  tbody.innerHTML = filteredRows.map((row, i) => {
    ...
    return `<tr id="row-${i}">
      ...
      <td>
        <input class="inline-input" value="${esc(row[C.responsible] || '')}" placeholder="שמך (המטפל)..."
...
function openModal(idx) {
  ...
  document.getElementById('e-internal').value = row[C.internalNotes] || '';
...
async function saveChanges() {
  ...
  const nn = document.getElementById('e-internal').value;

  btn.disabled = true; btn.textContent = 'שומר...';
  msg.textContent = ''; msg.className = 'save-msg';

  if (!LIVE) {
    row[C.status] = ns; row[C.responsible] = nr; row[C.internalNotes] = nn;
  ...
  try {
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      const batch = rows.slice(i, i + BATCH).map(r =>
        [r.id, r.title, r.desc, r.address, r.area, r.contactName, r.phone ? "'" + r.phone : "", r.status,
